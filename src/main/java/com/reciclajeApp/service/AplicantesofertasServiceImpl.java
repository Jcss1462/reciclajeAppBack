...
public class AplicantesofertasServiceImpl implements AplicantesofertasService {
	...
	public void validate(Aplicantesofertas entity) throws Exception {
		...
		if (usuarioRepository.findById(entity.getEmail_reciclador().getEmail()).get().getTipousuario().getIdtipousuario()!=1) {
			throw new Exception("El usuario : " + entity.getEmail_reciclador().getEmail() + " no es reciclador, solo los usuarios recicladores pueden ofertar");
		}
